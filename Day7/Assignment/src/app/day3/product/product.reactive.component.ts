...
import { EvenValidator } from '../even.validator';
@Component({
...
export class ProductReactiveComponent implements OnInit {
  ...
  set searchText(srch:string){

    this._searchText = srch;
    this.search(srch);
  }
  ...
  search(search: string){
    ...
    this.products = this.products.filter((v: Product) =>
     {
      const index =  v.ProductName.toUpperCase().indexOf(search.toUpperCase());
      console.log(index);
      return index >= 0;
      });
  }
